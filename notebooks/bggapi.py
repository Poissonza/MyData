...
class BGGAPI:
    ...
    def get_plays(selfs, game_id: int, cache_location: pathlib.Path):
        ...
        if id_collected_file.is_file():
            ...
            id_collected = id_collected_json[str(game_id)]
        else:
            id_collected_json = {}
            id_collected = []
        ...
        if not str(game_id) in last_date:
            last_date.update({str(game_id): "0000-00-00"})
        ...
        url_data = urllib.request.urlopen(
            f"{play_url}&mindate={last_date[str(game_id)]}"
        ).read()
        ...
        for page in tqdm(range(max_pages, 0, -1), maxinterval=max_pages, desc="Page"):
            url_data = urllib.request.urlopen(
                f"{play_url}&mindate={last_date[str(game_id)]}&page={page}"
            ).read()
                ...
                if not int(play["id"]) in id_collected:
                    ...
                    if not play.find("players") == None:
                        ...
                        for player in play.players.findAll("player"):
                            dict.update(
                                {
                                    f"player_{player_number}_team": player["color"],
                                }
                            )
                            try:
                                dict.update(
                                    {
                                        f"player_{player_number}_score": int(
                                            player["score"]
                                        ),
                                    }
                                )
                    ...
                    last_date.update({str(game_id): play["date"]})
